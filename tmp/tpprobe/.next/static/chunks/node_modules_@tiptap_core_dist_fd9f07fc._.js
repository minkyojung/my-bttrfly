...
var extendMarkRange = (typeOrName, attributes)=>(param)=>{
        let { tr, state, dispatch } = param;
...
function isNodeViewSelected(param) {
    let { selection, pos, nodeSize, selectedOnTextSelection = false } = param;
        ...
        if ((isInsideExistingList || hasWholeDocSelectedList) && currentList) {
            if (currentList.node.type === listType) {
                if (isAllSelection && hasWholeDocSelectedList) {
                    return chain().command((param)=>{
                        let { tr: trx, dispatch: disp } = param;
...
function renderNestedMarkdownContent(node, h2, prefixOrGenerator, ctx) {
    ...
    let output = "".concat(prefix).concat(mainContent);
